...
function toFormState(workout: WorkoutDetail): FormState {
  return {
    name: workout.name ?? "",
    // Use UTC ISO slice for SSR consistency; corrected to local time via useEffect
    startedAt: workout.startedAt.toISOString().slice(0, 16),
    exercises:
      workout.exercises.length > 0
        ? workout.exercises.map((ex) => ({
            name: ex.name,
            sets: ex.sets.map((s) => ({ reps: String(s.reps), weight: s.weight })),
          }))
...
export function EditWorkoutForm({
  ...
  // Correct startedAt to local time after hydration (toISOString is UTC, format uses local tz)
  useEffect(() => {
    setForm((prev) => ({
      ...prev,
  ...
  function removeSet(i: number, j: number) {
    setForm((prev) => {
      ...
      exercises[i] = {
        ...exercises[i],
        sets: exercises[i].sets.filter((_, idx) => idx !== j),
      };
      return { ...prev, exercises };
    });
  }
